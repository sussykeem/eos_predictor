...
class Unet(nn.Module):
    def __init__(self, dataloader, patch_size=16, mask_ratio=0.75, enc=False):
        ...
        self.attention1 = SelfAttention(32, self.device)
        self.attention2 = SelfAttention(64, self.device)
        ...
        self.dropout1 = nn.Dropout(p=0.4)  # Add this dropout layer
        ...
        self.dropoutu = nn.Dropout(p=0.3)  # Add this dropout layer
        self.upscale4 = self.upscale_block(128, 64)
        self.decoder4 = self.conv_block(64, 64, 1, pool=False)
        self.upscale3 = self.upscale_block(64, 32)
        self.decoder3 = self.conv_block(32, 32, 2, pool=False)
    ...
    def mask_input(self, x):
        ...
        num_patches_per_row = width // self.patch_size
        ...
        for i in range(batch_size):
            ...
            for pos in mask_positions:
                row = pos // num_patches_per_row
                col = pos % num_patches_per_row
    ...
    def forward(self, x, enc=None):
        ...
        # chn2 = self.attention1(chn2)
        enc2 = self.encoder2(enc1)
        chn3 = self.channel3(enc2)
        # chn3 = self.attention2(chn3)
    ...
    def compute_loss(self, output, target, mask, factor=100.0, edge_factor=0.05):
        ...
        mse_loss  = self.foreground_weighted_mse(output, target)
        l1_fg_loss = self.foreground_l1_penalty(output, target)
        #mse_loss = F.mse_loss(output, target, reduction='mean')
        return factor * ((1-edge_factor) * mse_loss + (edge_factor) * l1_fg_loss)
    
    def foreground_weighted_mse(self, pred, target, background_val=0.25, fg_weight=5.0, bg_weight=1.0):
        """
        Compute weighted MSE that focuses on foreground but still penalizes background.
        """
        # Mask of pixels significantly different from background black
        fg_mask = (target > background_val).any(dim=1, keepdim=True).float()

        # Weight map: higher for foreground, but not zero for background
        weight = (fg_weight * fg_mask) + ((1 - fg_mask) * bg_weight)

        mse_per_pixel = (pred - target) ** 2
        weighted_mse = mse_per_pixel * weight

        return weighted_mse.mean()
    
    def foreground_l1_penalty(self, output, target, background_val=0.25, fg_weight=1.0, bg_weight=0.0):
        """
        Applies L1 loss only to the foreground, computed per-channel from the target.
        """
        # Foreground mask per-channel [B, 3, H, W]
        fg_mask = (target > background_val).any(dim=1, keepdim=True).float()
        weight = (fg_mask * fg_weight) + ((1 - fg_mask) * bg_weight)
        # Per-channel L1 loss
...
def unet_main():
    ...
    unet.save_model("unet2_sa_model.pth")
...
def load_encoder():
    seed_everything(42)
    data = EOS_Dataloader(mode='reconstruct', batch_size=64, num=1024)

    full_net = Unet(data)
    full_net.load_state_dict(torch.load('unet2_sa_model.pth', weights_only=True))
    full_net.eval()
